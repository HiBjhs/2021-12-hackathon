...
data_name = ["anger","contempt","disgust","fear",'sadness']#保存データの系列
emotion_data =[0,0,0,0,0]#初期値
count = 0#撮影回数を示すカウンタ

##顔認識の設定
cascade_path =  '/usr/local/opt/opencv/share/opencv4/haarcascades/haarcascade_frontalface_alt.xml'# 顔判定で使うxmlファイルを指定する。(opencvのpathを指定)
...
while True:
    ...
    if len(faces) > 0: #顔を検出した場合
        for face in faces:
            ...
            result = [analysis[0]['faceAttributes']['emotion']['anger'],analysis[0]['faceAttributes']['emotion']['contempt'],analysis[0]['faceAttributes']['emotion']['disgust'],analysis[0]['faceAttributes']['emotion']['fear'],analysis[0]['faceAttributes']['emotion']['sadness']]
